...
class vector
{
	...
	double y;
	//prototypes
	vector (double = 0, double = 0);   //initializer in case no argument
	vector operator + (vector);        //operator overload with class argument
	vector operator - (vector);        //operator overload with class argument
	vector operator - ();              //operator overload without argument
	vector operator * (double);        //operator overload with double argument
	double module();                   //double typed method without argument
	void set_length (double = 1);      //void typed method with double argument
};
